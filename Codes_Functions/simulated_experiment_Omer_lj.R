...
# ____ lj changed
# in seconds
dtDelay=c(-300,-233,-167,-100,-33,33,100,167,233,300)*(1/1000)
# in milliseconds
...
xtabs(~ delay + id, data=df)
xtabs(~ expectation + id, data=df)
xtabs(~ color + id, data=df)
...
delay_coeff=0.38
# ____ lj changed
#fixed <- c(0, 0.38, -0.5, 0.2)
fixed <- c(0, 0.4, -0.3, 0.2)


model <- makeLmer(y ~ delay*expectation +(1|id), fixef=fixed, VarCorr=0.1, sigma=1, data=df)# sigma=res,
model
summary(model)
...
data_from_model = getData(model)
head(data_from_model)

data_from_model.sum = group_by(data_from_model, id,delay,expectation) %>% summarize(sd_y = std.error(y),
                                                                                    y =mean(y))



plot_sim_data = ggplot(data = data_from_model.sum, aes(delay,y,color = as.factor(expectation), group = interaction(id,as.factor(expectation)))) +
  geom_point()+
  geom_line()+
  ...
  theme(axis.text.x = element_text(colour = "black",size=12), axis.text.y = element_text(colour = "black",size = 12),panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black",size=0.7),axis.ticks =element_line(colour = "black",size=0.7) , 
        axis.title.x= element_text(size = 14),axis.title.y = element_text(size = 14),
        legend.text=element_text(size=14),legend.title = element_text(size=14))
plot_sim_data


sim_power  = powerSim(model,test = fcompare(y ~ delay + (1|id)), nsim=100)
summary(sim_power)
print(sim_power)
...
p_curve_sim_expectation <- powerCurve(model, test=fcompare(y~ delay +(1|id)), along="id")
