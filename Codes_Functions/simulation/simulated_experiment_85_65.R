...
expectation_congruency_full=factor(c(moderate_expectation,high_expectation))
...
fixed= c(3.0,delay_coeff,expectation_coeff,expectation_coeff85)

fixed=c(5,delay_coeff,-0.7,0.03,0.4)
model <- makeLmer(y ~ delay*congruency+expectation +(1|id), fixef=fixed, VarCorr=0.1, sigma=0.1, data=df)# sigma=res,
...
sim_expectation <- powerSim(model, nsim=100, test = fcompare(y ~ delay))
sim_expectation
#Predicted results based on hard coded data
#####
p_curve_sim_expectation <- powerCurve(model, along="id")
